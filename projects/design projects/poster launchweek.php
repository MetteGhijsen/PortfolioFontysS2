<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link rel="stylesheet" href="../../css/posterlaunchweekproject.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.5.0/font/bootstrap-icons.css">
    <link rel="icon" href="../../img/logotjewit.png">
    <title>Portfolio Mette Ghijsen</title>
</head>

<body>
<div class="container">
    <div class="taakbalk">
        <li><a href="../../index.php"> Home </a></li>
        <li class="pijltje"> < </li>
        <li><a href="../../leeruitkomsten.php"> Leeruitkomsten </a></li>
        <li class="pijltje"> < </li>
        <li><a href="../../vakken/design.php"> Design </a></li>
        <li class="pijltje"> < </li>
        <li><a href="../../projects/design%20projects/poster%20launchweek.php" class="currentpage">Double Life Challenge</a></li>
    </div>

    <div class="hoofdtekst">
        <p>Poster Launchweek</p>
    </div>

    <div class="projectinfo">
        <p><i class="bi bi-pencil"></i> Design</p>
        <p><i class="bi bi-person-check-fill"></i> Individueel</p>
    </div>

    <div class="grid-container">
        <div class="text">
            <h1>Inleiding</h1>
            <p> Aan het begin van de launchweek kregen wij als klas de opdracht om een filmposter te maken over hoe ons
                leven eruit zou zien als we twee keer zo oud zouden zijn als nu.
                Wat gingen we doen in de toekomst? Welke leuke dingen waren er gebeurd? Hoe zou ons leven eruit zien? Ik
                vond het maar een lastige opdracht, en ik wist niet helemaal
                wat ik ermee aan moest.</p>
        </div>
        <div class="afbeelding">
            <img id="afbeelding1" src="../../img/design/posterV11.png" alt="projectimage">
        </div>
        <div class="afbeelding2">
            <img id="afbeelding2" src="../../img/design/posterV7.png" alt="projectimage">
        </div>
        <div class="afbeelding3">
            <img id="afbeelding3" src="../../img/design/posterV6.png" alt="projectimage">
        </div>
        <div class="afbeelding4">
            <img id="afbeelding4" src="../../img/design/posterV16.png" alt="projectimage">
        </div>
        <div class="text2">
            <h1>Ervaring</h1>
            <p> Toen ik eenmaal mijn poster bestand in Photoshop aanmaakte heb ik echt een tijdje naar de lege pagina
                gestaard. Ik wist totaal niet wat ik wilde. Op een gegeven moment ben ik toen begonnen met het maken van
                wat schetsen met mijn tekentablet. Ik zocht een coole manier om mijn naam te branden. Het leek me enorm
                vet om zelf een lettertype te maken, en aangezien ik een hele week de tijd had voor dit project leek me
                dat ook wel haalbaar. Na een beetje te hebben nagedacht over hoe ik mijn naam wilde branden, stuitte ik
                per toeval op het lettertype “Scream Real” van de film “Scream”. Omdat de M van dit lettertype verder
                naar beneden gaat, en mijn naam begint met een M, realiseerde ik me dat dit het perfecte lettertype was
                om mijn naam mee te branden. Toen ik eenmaal had besloten dat ik dit ging gebruiken, kwam de rest van de
                poster een beetje vanzelf. Als eerste heb ik de 2039 toegevoegd. Het is nu 2021, en wanneer ik twee keer
                zo oud ben als nu, is het 2039. Om het jaartal een beetje te scheiden van de rest van de tekst, had ik
                besloten er een subtiel streepje onder te zetten. Dit zag er mooi uit, en hier was ik tevreden mee. De
                rest van de lijnen op de poster zijn allemaal met de hand getekend met mijn tekentablet, en de teksten
                onderin de poster heb ik aan het einde nog toegevoegd zodat het echt op een filmposter leek. Ik besloot
                ook nog bij de acteurs wat vrienden van mij neer te zetten, omdat ze hierom hadden gevraagd.<br></br>

                Ondanks dat ik erg tevreden was met de simplistische lijnkunst die ik had gemaakt, had ik wel enige twijfels. De poster zegt op het eerste oog niet echt iets over mij, en ik dacht dat ik misschien mijn gezicht op de poster moest zetten om hem wat herkenbaarder te maken. Na een aantal versies te hebben geprobeerd, kwam ik toch tot de conclusie dat ik de originele rood met blauwe poster toch mooier vond.

                Ik ben erg tevreden met de poster, en ik denk toch dat mijn persoonlijkheid hier goed in naar voren
                komt. Ik weet namelijk niet zo goed wat ik aan het doen ben, of wat ik nou eigenlijk wil doen. Maar
                zolang ik dingen kan maken waar andere mensen blij van worden, ben ik ook blij, en dat is wat ik een
                beetje in deze poster naar voren wilde laten komen.
            </p>
        </div>
        <div class="text4">
            <h1>Wat heb ik geleerd?</h1>
            <p>Ik heb geleerd dat het oke is om op een andere manier naar je design te kijken, en een beetje outside the box te denken.
            Ik was heel onzeker over of mijn poster aan de opdracht voldeed, maar te merken aan de reacties van mijn docenten vonden
            ze het juist interessant hoe ik het heb aangepakt. Ik denk ook dat ik de volgende keer beter in Illustrator kan werken,
            en van de poster een SVG moet maken in plaats van een PNG. Hoewel ik hier geen feedback over heb gekregen, verwacht
            ik wel dat docenten hier blijer mee zullen zijn in de toekomst. Ik moet echter wel nog leren hoe Illustrator werkt.</p>
        </div>
        <div class="text5">
            <h1>Feedback</h1>
            <p>De feedback die ik kreeg was erg positief, en daar was ik eigenlijk wel verbaasd over. Hoewel ik erg blij
                was met mijn poster,
                had ikzelf niet het idee dat hij volledig voldeed aan de opdracht. Docenten zeiden dat ze mijn design
                heel mooi in balans vonden,
                het leek alsof ik echt wist was ik aan het doen was. Het is fijn aan het oog, en hoewel je niet echt kan
                zien waar de film over gaat
                wekt de poster toch genoeg nieuwsgierigheid op zodat je gelijk een ticket wilt kopen. Verder vonden ze
                het cool wat ik met de branding van mijn
                naam had gedaan. De punt in de "M" vonden ze er erg mooi uit zien, en het bescheiden streepje onder de
                "2039" vonden ze ook een aangename
                subtiele touch. Een van mijn medestudenten had ook nog de makers van mijn film opgemerkt, en aangegeven
                dat hij het erg grappig vond.
                Ik was erg blij dat iedereen zo positief was, en ik kreeg er echt een goed gevoel van. </p>
        </div>
    </div>
    <div class="download">
        <h2>Klik hier om het project te downloaden</h2>
        <a href="../../img/design/posterV11.png" download="posterV11.png">
            <p><i class="bi bi-download"></i></p>
        </a>
    </div>
</body>

<footer>
    <div class="copyrightnote">
        <p>Copyright ©2021 By Mette Ghijsen, All Rights Reserved</p>
    </div>
</footer>
</html>
